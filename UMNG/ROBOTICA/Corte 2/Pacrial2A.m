%% Es necesario primero correr el startup de robotic toolbox de peter corkce
% Andres Felipe Bernal Urrea - 7003748
% Filocaris Triana Pinzon - 7003936
run('C:\Users\pinzo\Documents\GitHub\UMNG\ROBOTICA\toolbox MATLAB\RVC2-copy\RVC2-copy\rvctools\startup_rvc.m')
clear;clc;close all;

% Definicion del modelo dimplificado 3 Grados PRP
L = [200 300 200];
...
% 6 Grados de Libertad PRP-MUÑECA
...
LPW = [ 0; 0; L6(5) + L6(6)]; %Punto Muñeca
home = [0 0 0 0 0 0]; %Vector articular de referencia (estado 0)

QEF = [-170 20 170];        % Angulo deseado de la herramienta en la posicion deseada  [Y P R] (DIPLAY PETER CORKE)
RT = rot(QEF);
QEFH = [180 0 -180];       %Angulo en de la herramienta en posicion Home
RTH = rot(QEFH);
...
R3_6QH = R0_3QH \ RTH;
eulQH = rotm2eul(R3_6QH, "ZXZ"); %Matriz Euler ZXZ

T0_3Q1 = Rob3r.fkine(Q1);
R0_3Q1 = [T0_3Q1.n T0_3Q1.o T0_3Q1.a];
R3_6Q1 = R0_3Q1 \ RT;
eulQ1 = rotm2eul(R3_6Q1, "ZXZ"); %Matriz Euler ZXZ

T0_3Q2 = Rob3r.fkine(Q2);
R0_3Q2 = [T0_3Q2.n T0_3Q2.o T0_3Q2.a];
R3_6Q2 = R0_3Q2 \ RT;
eulQ2 = rotm2eul(R3_6Q2, "ZXZ"); %Matriz Euler ZXZ

T0_3Q3 = Rob3r.fkine(Q3);
R0_3Q3 = [T0_3Q3.n T0_3Q3.o T0_3Q3.a];
R3_6Q3 = R0_3Q3 \ RT;
eulQ3 = rotm2eul(R3_6Q3, "ZXZ"); %Matriz Euler ZXZ

T0_3Q4 = Rob3r.fkine(Q4);
R0_3Q4 = [T0_3Q4.n T0_3Q4.o T0_3Q4.a];
R3_6Q4 = R0_3Q4 \ RT;
eulQ4 = rotm2eul(R3_6Q4, "ZXZ"); %Matriz Euler ZXZ

% Vectores Articulares Completos 
QHT = [QH eulQH];
Q1T = [Q1 eulQ1];
Q2T = [Q2 eulQ2];
Q3T = [Q3 eulQ3];
...
Rob6r.teach(Q1T)
hold on
plot3(PHD(1),PHD(2),PHD(3),'k*') %Punto Home 
plot3(P1D(1),P1D(2),P1D(3),'r*') %PD1
plot3(P2D(1),P2D(2),P2D(3),'b*') %PD2
plot3(P3D(1),P3D(2),P3D(3),'g*') %PD3
plot3(P4D(1),P4D(2),P4D(3),'y*') %PD4


% Linear Interpolation in the space of joint variables ANIMATION PD1
...
pause(5); % Pausa en PD deurante 5 segundos
% Linear Interpolation in the space of joint variables PD HOLD ON
...
pause(5);
cla; % Clear Axis (Resetea el plot de la trayectoria)
%Plotea Nuevamente los puntos deaseados
...
% Linear Interpolation in the space of joint variables ANIMATION PD2
...
% Linear Interpolation in the space of joint variables ANIMATION PD3
...
% Linear Interpolation in the space of joint variables ANIMATION PD4
...
function T = rot(QD) % QD = [alpha beta gamma]
    ...
    T = Rx * Ry * Rz; %(multiplicación de rotaciones elementales X, Y, Z)
end
%PRP
