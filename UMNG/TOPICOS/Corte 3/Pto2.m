%% Definición de la planta y discretización
clc; clear; close all;

% 1. Definir el tiempo de muestreo
Tm = 0.25;

% 2. Definir la variable de Laplace 's'
s = tf('s');

% 3. Definir la Función de Transferencia continua G(s) = 1/s^2
G = -1/(s^2-1);

% 4. Convertir a discreto usando c2d (Continuo a Discreto)
% 'zoh' significa Zero-Order Hold (el método por defecto)
Gd = c2d(G, Tm, 'zoh');

% 5. Mostrar resultados
disp('--- Planta Continua G(s) ---');
G
disp('--- Planta Discreta G(z) con Tm = 0.05 ---');
Gd

% 6. (Opcional) Verificar numerador y denominador
[num, den] = tfdata(Gd, 'v');
disp('Coeficientes del numerador y denominador discretos:');
disp(['Num: ', num2str(num)]);
disp(['Den: ', num2str(den)]);
...
G = [0 -1; 
     1  2.063];        % Matriz de estado
 H = [-0.03141; 
     -0.03141];       % Matriz de entrada
...
Gep = [G  zeros(2,1); 
       -C*G  1]   % Matriz extendida de estados

Hep = [H; 
       -C*H]      % Vector extendido de entradas

% Matriz de controlabilidad extendida
Sep = [Hep  Gep*Hep  Gep^2*Hep]  % hasta orden n (aquí n=3)

% Parámetros de diseño
zeta = 0.5;      % coeficiente de amortiguamiento %1
Ts   = 5;      % tiempo de asentamiento deseado (2%) 2
Tm   = 0.25;    % tiempo de muestreo 0.1
...
POLD = expand((z^2 + P1*z + P2) * (z + alpha));
...
I = [1 0 0; 0 1 0; 0 0 1]; %Identidad  3x3
fiG = Gep^3 - 1.64*Gep^2 +0.824*Gep -0.067*I % cambiar por lo que del POLD

K = [0 0 1]*inv(Sep)*fiG %Ackerman
