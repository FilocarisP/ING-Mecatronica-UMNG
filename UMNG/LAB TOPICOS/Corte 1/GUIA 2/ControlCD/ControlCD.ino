...
void periodic_timer_callback(void* arg) {
  ...
  float ref_env   = entrada;          // referencia (ángulo deseado)
  float sal_env   = theta;            // salida (ángulo medido)
  float ctrl_env  = uk;               // señal de control calculada

  // Empaquetar los 4 floats en un array de bytes
  byte buffer[16];
  ...
  SerialBT.write(buffer, 16);  // ← envío directo en binario
