...
void Control(float ang, float in) {
  ...
  static unsigned long tiempoPerdida = 0;
  ...
  // ===============================================================
  //       ECUACIÓN EN DIFERENCIAS DEL CONTROLADOR DISCRETO DISCREO
  // ===============================================================
  // ---- Nuevo controlador discreto (normalizado) ----
  const float b0 = 41.638f;
  const float b1 = -4.2746f;
  const float b2 = -37.362f;
  const float a1 = -(-299.73f);  // signo invertido por paso a diferencias
  const float a2 = -298.73f;

  // Ecuación: y[k] = -a1*y[k-1] - a2*y[k-2] + b0*u[k] + b1*u[k-1] + b2*u[k-2]
  float y = -a1 * y1 - a2 * y2 + b0 * u + b1 * u1 + b2 * u2;

  // Limitar el control angular (°)
  y = constrain(y, -14.0f, 14.0f);
  ...
  // ============================================================
  //       CONTROL DE VELOCIDAD Y DIRECCIÓN DEL ROBOT
  // ============================================================

  int velocidad;
  const float k_vel = 5.0f;
  const float umbralCurva = 1.05f;
  ...
  // ============================================================
  //       ACCIÓN DE CONTROL SOBRE LOS MOTORES
  // ============================================================
  if (error >= 6.0f) {
    ...
    // ==== Recuperación si se pierde la línea ====
    if (tiempoPerdida == 0) tiempoPerdida = millis();

    if (millis() - tiempoPerdida <= 1500) {
      if (ultimaDireccion == +1) cerrar_derecha(v_min);
      else if (ultimaDireccion == -1) cerrar_izquierda(v_min);
      else parar();
    } else {
      parar();
      ultimaDireccion = 0;
...
void periodic_timer_callback(void* arg) {
  ...
  float error_env = entrada - theta;  // señal de error
  float ref_env   = entrada;          // referencia
  float sal_env   = theta;            // salida
  float ctrl_env  = uk_;      // control
